...
// PORTED from apps/mobile/src/render/buildCard.ts (M4 render spike, decision 0064) for the M5 §1
// server-side flatten. The M4 module is deliberately source-agnostic (the skia component set is passed
// in), so this is a VERBATIM copy — the only change is the `steppedPath` import path (colocated here).
// DESIGN SMELL / P3: this duplicates ~900 lines with the mobile module. The clean fix is extracting
// buildCard + steppedPath + icons + the render-composition type into a shared `@ingame/card-render`
// package both surfaces import — deferred (it re-points the signed M4 mobile import paths). See report.
