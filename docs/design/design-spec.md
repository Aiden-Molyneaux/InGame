...
**States & feedback (§1.6):** `EmptyState/inviting` · `Skeleton` (solid fills) · `LoadError` ("Signal Lost" + RETRY) · `Unavailable` (terminal — person glyph, no retry, MOD-09) · `Offline` + `OfflineStrip` (calm, SYS-10).
**To design (Phase C/D gaps):** text inputs (multi-line/numeric) · dropdown/picker/slider/toggle · segmented control (Discover needs it) · modals/confirm dialogs/sheets · toasts/banners · celebration moment · currency counter (`ECON-07`) · card back · `motion.cardFlip`/`powerOn`/`celebration`.
...
- **Transient** failures (save/publish/purchase) → toast/banner + retry *(component pending)*.
...
**Still open** (owed before the screens that touch them): OQ-007 (break-out → Card editor) · OQ-005 (egg presentation → Achievements) · OQ-038 (offline cache scope, SYS-10).
