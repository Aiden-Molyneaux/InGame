...
| `friends-draft-b-roster.html` | **B · Roster-first** | *(building)* The friends **list is the body** (presence-sorted + a tool bar); feed = a collapsed `RECENT` digest, requests = a count-badged header key. The people-directory pole. |
| `friends-draft-c-connect.html` | **C · Connect-first** | *(building)* A priority-stacked landing — **requests + the `InviteHook`** (search · link · QR → 4.8) lead as the hero; roster + an activity digest sit beneath. Tuned to the cold-start / low-friend reality. The growth pole. |
