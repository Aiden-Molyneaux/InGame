...
- OQ-135: **nameplate shape NONE renders a card face with no title at all — should the CARD-01 name guarantee survive it?** Decision 0063 §4 sanctions a `none` plate shape and the Styler implements it faithfully (post-fix, NONE keeps the title/font/ink in the composition document but draws neither plate nor title), yet the styler board hints "THE NAME ALWAYS RENDERS (CURATED SHAPES)" (`styler-states.html:907`) and CARD-01 leans legibility-first. Owner/spec ruling: is a titleless face acceptable taste, or does plate-NONE owe a plateless floating title (the "curated shapes" reading)? (raised by parvati reviewing §3.2 Styler, 2026-07-06) [undecided]
