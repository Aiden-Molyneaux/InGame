...
**Tally:** 🚩 5 flag · ✅ 6 expected · 🎨 3 polish. *(Updated after the owner's Collection + Profile passes, the Burt→Parvati transition, and the presence≠placement sharpening.)*
- 🚩 **FLAG (owed at M2):** P1 bio · P4 gamertags (contract-fed identity the render drops) · **P5 device-hero size/label** (screen-width + unlabeled — should be a small "MY DEVICE" mini-dev) · **P8 profile section-order scrambled** · C1 collection view-switch docked top-not-bottom.
...
- 🎨 **POLISH / iteration** (Parvati surfaces built-app DS; Burt mockup-only + dormant): P5 device F-03 render/frame · **P9 stats not centered+spanning** · C2 control-bar styling.
