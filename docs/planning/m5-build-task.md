...
- [ ] **P2b + G-J + sandbox pass** — the one remaining-open item, explicitly recorded as owed on provisioning (§6): **rides Google Play verification — carries into the M6 window per §5/§6** (`react-native-purchases` uninstalled; RC webhook unwired; provisioning-log #6/#7/#8; recorded in `m5/economy-receipt.md`)
- [x] P3 built · **G-D re-fire RUN LIVE with the owner 2026-07-14 — PASSED, and it BIT**: the predicate strip turned the standing runtime test RED (drafts leaked → caught), while exposing a lint hole (the ±16-line predicate window matched a SIBLING read's `publishedOnly(` — neighbor-laundering) → fixed same-day (statement-scoped detection, +3 rule tests, the exact miss as fixture `public-read-neighbor-launder-repo.ts`; RED proof now = 2 fail-closed violations) · publish→gallery→adopt walked cross-user · contributor stats live-verified — **DONE: the owner-watched G-D re-fire sitting was sat 2026-07-14** (`08e42f7`; the sibling-neighbor lint hole it caught was fixed same-day) · P3 cross-user walked (P7/P8 parvati) · stats live-verified
...
- [x] P7 reconcile/publish/premium rows landed on all three editors · no regression on signed M4 behavior (murr diff lane) · owner walked reconcile + publish ritual — **DONE: owner device walk PASSED 2026-07-15** (P7 built + murr-diff-guarded + parvati; the reconcile funded path + publish ritual motion cleared across the three walk rounds → F-8…F-21; see `m5/surfaces-receipt.md` Addendum)
- [x] P8 gallery+adopt through the pipeline · owner walked browse→adopt→equip both price paths — **DONE: owner device walk PASSED 2026-07-15** (P8 built + parvati 1 🚩 → F-2/F-2b fixed the equip; browse→adopt→equip + hold-to-buy feel accepted on device across walk rounds 2–3 → F-12…F-21)
