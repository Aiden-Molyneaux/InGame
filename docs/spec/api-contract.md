...
**Version:** 0.25 (draft) · **Last updated:** 2026-06-24 · **Owner:** Claude Code
...
| Method | Path | Notes |
|---|---|---|
| GET | `/games/:gameId/cards` | Published community cards for a game (gallery); each card object carries a read-only **`equipped`** readout — `{ base?, effect?{ name, intensity }, finish?, frame?, nameplate?, font? }` — **display labels/ids, not the composition JSON** (CARD-22). This `equipped` summary **— and the card's `designer { userId, username }` attribution (CARD-01/04, the card-back "CARD ART DESIGNED BY" provenance; COL-12) —** **rides every card payload** (gallery · the switcher `/me/collection/:entryId/cards` · the collection-item `card` on `/me/collection` + `/users/:id/collection`) |
