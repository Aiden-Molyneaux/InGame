...
**Version:** 0.23 (draft) · **Last updated:** 2026-06-18 · **Owner:** Claude Code
...
| ID | Pri | Behavior |
|---|---|---|
| CARD-01 | P0 | A **Card editor**, launched for a specific game, composes the card **front** as layers (the **back is a standardized** auto-stats layout **incl. printed provenance — DESIGNER ATTRIBUTION ONLY** ("CARD ART DESIGNED BY" + creator name; **the adoption count is NOT printed on the back** — it surfaces only in the community gallery (`AdoptCount`, CARD-05) + the `CardDetail` inspect sheet. Narrows decision 0015's "designer + adoption count on the back"; the back is the owner's clean trophy face — decision 0024, OQ-069): *optional colour/gradient base → vector elements → one animated effect → finish → frame → **nameplate** → title (**font + ink**).* Trading-card portrait. *(Nameplate = the plate **object** the title sits on — a cosmetic type per OQ-039/decision 0018; the title layer itself is font + ink colour.)* |
