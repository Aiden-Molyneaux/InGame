...
**Version:** 0.32 (draft) · **Last updated:** 2026-06-27 · **Owner:** Claude Code
...
| ID | Pri | Behavior |
|---|---|---|
| AUTH-01 | P0 | Email + password registration; **email is required and unique**; passwords are required (**minimum 8 characters**) and hashed (argon2/bcrypt). |
...
| AUTH-11 | P1 | **Username-availability pre-check:** a public (pre-auth) endpoint reports whether a candidate username is free **and passes screening** (MOD-07), so create-account / SIWA-completion (AUTH-09) can show a live availability beat instead of failing at submit. **Advisory** — uniqueness + screening stay authoritatively enforced at register/PATCH; rate-limited (SYS-05). (Decision 0029.) |
...
| CAT-09 | P1 | **Community presence stats** on every catalog entry: **(a) collections count** — how many users' collections contain the game (anonymous aggregate, honest at any size — no threshold gating needed, unlike PROF-07's percentiles); **(b) friends-have-it count** — how many of *the viewer's* friends have it (derives from existing friend visibility, PROF-03/COL-10; blocks sever friendships, so SOC-09 needs no special-casing). Surfaced in **catalog search results** (the add flow's focused-card meta + card detail) and on the **Game page**. *(Pulls the presence counts forward from the Game page's "community aggregate stats — later phase"; richer aggregates — average hours, completion rates — stay parked for later.)* **The collections-count also ranks the "popular" suggestion source** (Add Game empty-state + onboarding, AUTH-06): top-by-count, **capped ~12, no paging** (the rail is a nudge — Discover is the browse surface). (OQ-051) |
...
| CARD-05 | P1 | A card tracks **adoption count / popularity**; the editor surfaces a creator's adoptions, **clout**, and **milestone progress** (ECON-05, ACH) — a lightweight creator dashboard. **There is NO card rating anywhere — individual or aggregate. Adoption-count is deliberately the card's ONLY social signal** (ECON-05 — creators earn clout, pointedly not ratings); a card rating is **pinned out** so a competing popularity metric isn't introduced later (decision 0024, OQ-058). *(The per-game personal rating, COL-03, is a different thing — the user's private ⭐ for the GAME, not the card.)* |
...
| CARD-14 | P0 | **Drafts & lifecycle:** an explicit **Draft** state + drafts shelf; **autosave + crash recovery**; **unsaved-exit guard**; **duplicate / save-as-copy**. A **saved-private** card surfaces in **both** the game's card switcher (COL-06) **and** the My-designs shelf (drafts · private · published) — no separate surface. (OQ-049) |
| CARD-15 | P0 | **Render/publish pipeline:** the editable **composition (JSON)** is **flattened to a static image** (thumbnail + full) on the CDN at save/publish — **viewers download one image, not the layers**; the **effect + finish render as runtime overlays**; **element count is capped** (server-configurable, SYS-04; **starts at 30**, OQ-008). A **true-to-life preview** (flattened + overlays + thumbnail safe-area) precedes publish. Rendering via react-native-skia. |
...
| MOD-04 | P1 | An in-app **Admin console** (gated to `adminTier ≥ 1`, SYS-08; decisions 0033/0034) hosting the **P1 Content + P2 Catalog** capabilities — reports queue, edit-suggestion review (MOD-06), catalog dedup/merge (MOD-05) + the in-app remediation additions (OQ-081). **Reached via Settings → Admin console** (a role-gated entry; renamed from "Mod/Admin" — moderator dropped, 0034). The reports queue is **split by target type** — card / game / user — into separate but equivalent screens (user-report screens carry the MOD-09 suspension action). **Tiers I/II live here; III/IV's economy/config/governance (P3–P5) live in the external operator tool** (§10/OQ-080) — money, config, and role-granting never enter the phone app. **Admin actions are logged** (actor · action · target · reason · time — generalizing MOD-09's "logged"); the **audit-ledger viewer is out of v2 scope** (P5, parked §10). |
